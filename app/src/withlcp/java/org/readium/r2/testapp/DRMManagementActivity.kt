...
class DRMManagementActivity : AppCompatActivity(), CoroutineScope {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    lcpService.retrieveLicense(
      intent.getStringExtra("publication")
        ?: throw Exception("publication required"), null
    ) { license, error ->
      license?.let {
        drm.license = license
    ...
    val adapter = object :
      ArrayAdapter<Int>(this@DRMManagementActivity, R.layout.item_spinner_days, daysArray) {
              ...
              button {
                ...
                onClick {
                  ...
                  if ((drm.license as LCPLicense).status?.link(StatusDocument.Rel.renew)?.type == "text/html") {
                    ...
                    intent.data =
                      Uri.parse((drm.license as LCPLicense).status?.link(StatusDocument.Rel.renew)?.href.toString())
