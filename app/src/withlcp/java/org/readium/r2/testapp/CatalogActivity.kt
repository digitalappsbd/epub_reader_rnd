...
class CatalogActivity : LibraryActivity(), LCPLibraryActivityService, CoroutineScope,
  DRMLibraryService, LCPAuthenticating, LCPAuthenticationDelegate {
  ...
  override fun fulfill(byteArray: ByteArray, completion: (Any?) -> Unit) {
    lcpService.importPublication(byteArray, this) { result, error ->
      result?.let {
        val publication = DRMFulfilledPublication(
  ...
  override fun requestPassphrase(
            ...
            if (result is Exception) {
              ...
              result?.let {
                ...
                launch {
                  ...
                  pub?.let {
                    val pair =
                      parser.fillEncryption(pub.container, pub.publication, pub.container.drm)
    ...
    loadPublication(file.absolutePath, drm) {
      launch {
        ...
        if (it is Exception) {
          ...
          this@CatalogActivity.startActivity(
            intentFor<EpubActivity>(
              "publicationPath" to publicationPath,
              "publicationFileName" to book.fileName,
