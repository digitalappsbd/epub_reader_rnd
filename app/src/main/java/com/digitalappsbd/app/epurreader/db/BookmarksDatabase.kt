package com.digitalappsbd.app.epurreader.db
...
class BookmarksDatabaseOpenHelper(ctx: Context) :
  ManagedSQLiteOpenHelper(ctx, "bookmarks_database", null, DATABASE_VERSION) {
      ...
      1 -> {
        try {
          ...
          task {
            ...
            val cursor = db.query(
              BOOKMARKSTable.NAME,
              arrayOf(BOOKMARKSTable.ID, "progression", BOOKMARKSTable.LOCATION),
              null,
              null,
              null,
              null,
            ...
            if (cursor != null) {
              ...
              while (hasItem) {
                ...
                values.put(
                  ...
                  Locations(progression = progression).toJSON().toString()
                )
                db.update(
                  BOOKMARKSTable.NAME,
                  values,
  ...
  private fun has(bookmark: Bookmark): List<Bookmark> {
    return database.use {
      select(
        ...
        .whereArgs(
          "(bookID = {bookID}) AND (publicationID = {publicationID}) AND (resourceIndex = {resourceIndex}) AND (resourceHref = {resourceHref})  AND (resourceHref = {resourceHref}) AND (location = {location}) AND (locatorText = {locatorText})",
  ...
  class MyRowParser : RowParser<Bookmark> {
    override fun parseRow(columns: Array<Any?>): Bookmark {
      ...
      return Bookmark(
        ...
        resourceHref as String,
        resourceType as String,
        resourceTitle as String,
        Locations.fromJSON(JSONObject(location as String)),
        LocatorText.fromJSON(JSONObject(locatorText as String)),
        created as Long,
        id as Long
      )
    }
