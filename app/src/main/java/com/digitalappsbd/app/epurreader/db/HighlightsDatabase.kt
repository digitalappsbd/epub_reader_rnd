...
class HighlightsDatabaseOpenHelper(ctx: Context) :
  ManagedSQLiteOpenHelper(ctx, "highlights_database", null, DATABASE_VERSION) {
...
class HIGHLIGHTS(private var database: HighlightsDatabaseOpenHelper) {
  ...
  fun delete(highlight_id: String?) {
    highlight_id?.let {
      database.use {
        delete(
          HIGHLIGHTSTable.NAME, "${HIGHLIGHTSTable.HIGHLIGHT_ID} = {highlightID}",
  ...
  fun listAll(bookID: Long, resourceHref: String): MutableList<Highlight> {
    return database.use {
      select(
        ...
        .whereArgs(
          "${HIGHLIGHTSTable.BOOK_ID} = {bookID} AND ${HIGHLIGHTSTable.RESOURCE_HREF} = {resourceHref}",
