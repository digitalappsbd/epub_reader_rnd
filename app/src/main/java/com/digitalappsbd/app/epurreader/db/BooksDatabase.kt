...
class BooksDatabaseOpenHelper(ctx: Context) :
  ManagedSQLiteOpenHelper(ctx, "books_database", null, DATABASE_VERSION) {
  ...
  private fun upgradeVersion2(db: SQLiteDatabase, callback: () -> Unit) {
    ...
    val cursor =
      db.query(BOOKSTable.NAME, BOOKSTable.RESULT_COLUMNS, null, null, null, null, null, null)
    if (cursor != null) {
...
object BOOKSTable {
  ...
  var RESULT_COLUMNS =
    arrayOf(ID, IDENTIFIER, TITLE, AUTHOR, HREF, COVER, EXTENSION, CREATION, PROGRESSION)
  ...
  class MyRowParser : RowParser<Book> {
    override fun parseRow(columns: Array<Any?>): Book {
      ...
      return Book(
        ...
        cover,
        progression,
        Publication.EXTENSION.fromString(ext)!!
      )
    }
  }
}
