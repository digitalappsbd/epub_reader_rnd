...
import com.digitalappsbd.app.epurreader.db.PositionsDatabase
...
class R2SyntheticPageList(
  private val positionsDB: PositionsDatabase,
  private val bookID: Long,
  private val publicationIdentifier: String
) : AsyncTask<Triple<String, String, MutableList<Link>>, String, MutableList<Position>>() {
  ...
  private fun createSyntheticPages(baseURL: String, epubName: String, link: Link) {
    ...
    for (char in 0 until plainTextFromHTML.length - 1) {
      if (char % 1024 == 0) {
        resourcePageList.add(
          Position(
            pageNumber++,
            resourceHref,
            resourceType,
            char.toDouble() / plainTextFromHTML.length.toDouble()
          )
        )
      }
    }
  ...
  private fun addPages(resourcePageList: MutableList<Position>) {
    for (i in 0 until resourcePageList.size) {
      syntheticPageList.add(
        Position(
          resourcePageList[i].pageNumber,
          resourcePageList[i].href,
          resourcePageList[i].type,
          resourcePageList[i].progression
        )
      )
    }
  }
...
class Position(
  ...
  companion object {
    ...
    fun toJSON(
      ...
      syntheticPageList: MutableList<Position>
    ): JSONObject {
      val json = JSONObject()
