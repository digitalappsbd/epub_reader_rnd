...
import com.digitalappsbd.app.epurreader.epub.Position
...
class R2OutlineActivity : AppCompatActivity() {
        ...
        page_list.setOnItemClickListener { _, _, position, _ ->
          ...
          intent.putExtra(
            ...
            Locator(
              ...
              resourceType,
              publication.metadata.title,
              Locations(progression = pageProgression),
              null
            )
          )
          setResult(Activity.RESULT_OK, intent)
   ...
   */
  inner class NavigationAdapter(var activity: Activity, private var items: MutableList<Any>) :
    BaseAdapter() {
  ...
  inner class SyntheticPageListAdapter(
  ...
  inner class BookMarksAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
      ...
      val formattedProgression =
        "${((bookmark.location.progression!! * 100).roundToInt())}% through resource"
