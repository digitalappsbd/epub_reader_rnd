package com.digitalappsbd.app.epurreader.library
...
import com.digitalappsbd.app.epurreader.utils.toFile
...
//        intent.putExtra(Intent.EXTRA_MIME_TYPES, mimeTypes)
  ...
  private fun parseIntent(filePath: String?) {
    ...
    filePath?.let {
      ...
      val uriString: String? =
        intent.getStringExtra(com.digitalappsbd.app.epurreader.utils.R2IntentHelper.URI)
      uriString?.let {
        when (Publication.EXTENSION.fromString(intent.getStringExtra(com.digitalappsbd.app.epurreader.utils.R2IntentHelper.EXTENSION)!!)) {
  ...
  fun prepareSyntheticPageList(pub: Publication, book: Book) {
    if (pub.pageList.isEmpty() && !(positionsDB.positions.isInitialized(book.id!!))) {
      ...
      when (pub.type) {
        Publication.TYPE.EPUB -> syntheticPageList.execute(
          Triple(
            "$BASE_URL:$localPort/",
            book.fileName!!,
            pub.readingOrder
        ...
        Publication.TYPE.WEBPUB -> syntheticPageList.execute(
          Triple(
            "",
            book.fileName!!,
            pub.readingOrder
    ...
    launch {
      ...
      val book: Book = when (publication.type) {
        Publication.TYPE.EPUB -> {
          preferences.edit()
            .putString("$publicationIdentifier-publicationPort", localPort.toString()).apply()
          val author = authorName(publication)
  ...
  override fun recyclerViewListLongClicked(v: View, position: Int) {
    val layout = LayoutInflater.from(this)
      .inflate(R.layout.popup_delete, catalogView, false) //Inflating the layout
  ...
  override fun recyclerViewListClicked(v: View, position: Int) {
    val progress =
      indeterminateProgressDialog(getString(R.string.progress_wait_while_preparing_book))
    progress.show()
    task {
      val book = books[position]
      val publicationPath = R2DIRECTORY + book.fileName
        ...
        Publication.EXTENSION.EPUB -> {
          ...
          pub?.let {
            pub.container.drm?.let { drm: DRM ->
              prepareAndStartActivityWithLCP(
                drm,
                pub,
                book,
                file,
  ...
  private fun prepareWebPublication(externalManifest: String, webPub: Book?, add: Boolean) {
    task {
      ...
      json?.let {
        ...
        val externalURIBase = externalPub.linkWithRel("self")!!.href!!.substring(
          0,
          externalManifest.lastIndexOf("/") + 1
        )
        val externalURIFileName =
          externalPub.linkWithRel("self")!!.href!!.substring(externalManifest.lastIndexOf("/") + 1)
        ...
        if (add) {
          ...
          externalPub.coverLink?.href?.let { href ->
            ...
            book = Book(
              title = externalPub.metadata.title,
              href = externalManifest,
              identifier = externalPub.metadata.identifier!!,
              cover = stream.toByteArray(),
              ext = Publication.EXTENSION.JSON
            )
  ...
  private fun prepareAndStartActivity(
    pub: PubBox?,
    book: Book,
    file: File,
    publicationPath: String,
  ...
  override fun prepareAndStartActivityWithLCP(
    drm: DRM,
    pub: PubBox,
    book: Book,
    file: File,
    ...
    listener?.prepareAndStartActivityWithLCP(
      drm,
      pub,
      book,
      file,
  ...
  override fun processLcpActivityResult(
    uri: Uri,
    it: Uri,
    progress: ProgressDialog,
    networkAvailable: Boolean
