...
class PositionsDatabaseOpenHelper(ctx: Context) :
  ManagedSQLiteOpenHelper(ctx, "positions_database", null, DATABASE_VERSION) {
...
class POSITIONS(private var database: PositionsDatabaseOpenHelper) {
    ...
    jsonPageList?.let {
      if (jsonPageList.has("pageList")) {
        ...
        for (i in 1 until pageList.length()) {
          ...
          if (jsonObjectBefore.getString("href") == href && jsonObjectBefore.getDouble("progression") <= progression && jsonObject.getDouble(
              "progression"
            ) >= progression
          ) {
  ...
  fun delete(bookID: Long?) {
    bookID?.let {
      database.use {
        delete(
          POSITIONSTable.NAME, "bookID = {bookID}",
          "bookID" to bookID
        )
      }
    }
