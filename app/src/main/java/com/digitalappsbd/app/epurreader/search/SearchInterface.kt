...
class MarkJSSearchEngine(private var listener: IR2Activity) : SearchInterface {
  ...
  override fun search(
    keyword: String,
    callback: (Pair<Boolean, MutableList<SearchLocator>>) -> Unit
  ) {
    val searchResult = mutableListOf<SearchLocator>()

    for (resourceIndex in 0 until listener.publication.readingOrder.size) {
      val fragment = ((listener.resourcePager?.adapter as R2PagerAdapter).mFragments.get(
        (listener.resourcePager?.adapter as R2PagerAdapter).getItemId(resourceIndex)
