...
public final class FBReader extends FBReaderMainActivity implements ZLApplicationWindow {
  ...
  public static void openBookActivity(Context context, Book book, Bookmark bookmark,
      ReaderLifeCycle lifeCycleListener) {
  ...
  private final BroadcastReceiver myPluginInfoReceiver = new BroadcastReceiver() {
    ...
    public void onReceive(Context context, Intent intent) {
      final ArrayList<PluginApi.ActionInfo> actions = getResultExtras(
          true).<PluginApi.ActionInfo>getParcelableArrayList(PluginApi.PluginInfo.KEY);
      if (actions != null) {
      ...
      if (data != null) {
        if (data.getScheme() != null && "content".equals(data.getScheme().toLowerCase())) {
          myBook = createBookForFile(
              ZLFile.createFileByPath(getFilePathFromContentUri(data, getContentResolver())));
        } else {
  ...
  protected void onCreate(Bundle icicle) {
    ...
    myFBReaderApp
        .addAction(ActionCode.SHOW_PREFERENCES, new ShowPreferencesAction(this, myFBReaderApp));
    ...
    myFBReaderApp
        .addAction(ActionCode.SHOW_BOOKMARKS, new ShowBookmarksAction(this, myFBReaderApp));
    ...
    myFBReaderApp
        .addAction(ActionCode.SHOW_NAVIGATION, new ShowNavigationAction(this, myFBReaderApp));
    ...
    myFBReaderApp
        .addAction(ActionCode.SELECTION_SHARE, new SelectionShareAction(this, myFBReaderApp));
    myFBReaderApp.addAction(ActionCode.SELECTION_TRANSLATE,
        new SelectionTranslateAction(this, myFBReaderApp));
    myFBReaderApp
        .addAction(ActionCode.SELECTION_BOOKMARK, new SelectionBookmarkAction(this, myFBReaderApp));
    myFBReaderApp
        .addAction(ActionCode.DISPLAY_BOOK_POPUP, new DisplayBookPopupAction(this, myFBReaderApp));
    myFBReaderApp
        .addAction(ActionCode.PROCESS_HYPERLINK, new ProcessHyperlinkAction(this, myFBReaderApp));
    ...
    myFBReaderApp
        .addAction(ActionCode.OPEN_START_SCREEN, new StartScreenAction(this, myFBReaderApp));
    ...
    myFBReaderApp
        .addAction(ActionCode.INSTALL_PLUGINS, new InstallPluginsAction(this, myFBReaderApp));
    myFBReaderApp.addAction(ActionCode.SWITCH_TO_DAY_PROFILE,
        new SwitchProfileAction(this, myFBReaderApp, ColorProfile.DAY));
    myFBReaderApp.addAction(ActionCode.SWITCH_TO_NIGHT_PROFILE,
        new SwitchProfileAction(this, myFBReaderApp, ColorProfile.NIGHT));
    ...
    if ((intent.getFlags() & Intent.FLAG_ACTIVITY_LAUNCHED_FROM_HISTORY) != 0) {
      ...
      final Runnable runnable = new Runnable() {
        public void run() {
          ...
          if (myFBReaderApp.getTextView().search(pattern,
  ...
  protected void onStart() {
    ...
    getCollection().bindToService(this, new Runnable() {
      public void run() {
        ...
        Objects.requireNonNull(myFBReaderApp.getViewWidget()).repaint();
      }
    });
    ...
    Config.Instance().runOnConnect(new Runnable() {
      public void run() {
        ...
        SetScreenOrientationAction
            .setOrientation(FBReader.this, zlibrary.getOrientationOption().getValue());
      }
    });
  ...
  protected void onResume() {
    ...
    SetScreenOrientationAction
        .setOrientation(this, getZLibrary().getOrientationOption().getValue());
  ...
  private BroadcastReceiver mySyncUpdateReceiver = new BroadcastReceiver() {
    public void onReceive(Context context, Intent intent) {
      myFBReaderApp
          .useSyncInfo(myResumeTimestamp + 10 * 1000 > System.currentTimeMillis(), myNotifier);
    }
  };
  ...
  public void hideDictionarySelection() {
    ...
    Objects.requireNonNull(myFBReaderApp.getViewWidget()).reset();
    myFBReaderApp.getViewWidget().repaint();
  }
