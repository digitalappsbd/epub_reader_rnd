...
public class HighlightFragment extends Fragment implements
    HighlightAdapter.HighLightAdapterCallback {
  ...
  public void onViewCreated(@NotNull View view, @Nullable Bundle savedInstanceState) {
    ...
    RecyclerView highlightsView = mRootView.findViewById(R.id.rv_highlights);
    Config config = AppUtil.getSavedConfig(getActivity());
    String mBookId = Objects.requireNonNull(getArguments()).getString(FolioReader.EXTRA_BOOK_ID);

    if (Objects.requireNonNull(config).isNightMode()) {
      mRootView.findViewById(R.id.rv_highlights).
          setBackgroundColor(ContextCompat.getColor(Objects.requireNonNull(getActivity()),
              R.color.black));
    }
    ...
    adapter = new HighlightAdapter(getActivity(), HighLightTable.getAllHighlights(mBookId), this,
        config);
