...
abstract class R2FragmentPagerAdapter(private val mFragmentManager: FragmentManager) :
  androidx.fragment.app.FragmentStatePagerAdapter(mFragmentManager) {
    ...
    if (state != null) {
      ...
      if (fss != null) {
        for (fs in fss) {
          mSavedStates.put(
            fs,
            bundle.getParcelable<Parcelable>(fs.toString()) as Fragment.SavedState
          )
        }
      }
